...
/** Creates a Camera instance (User first person camera) */
export default function Player (heightField: Heightfield, waterHeight: number) {
	...
	function updateManual (i: input.State, dt: number) {
		...
		state.yawVel = 0
		if (i.turnLeft) {
			state.yawVel = MAN_YAWVEL * TURN_SPEED
		} else if (i.turnRight) {
			state.yawVel = -MAN_YAWVEL * TURN_SPEED
		}
		state.yaw += state.yawVel * ft
		...
		state.pitchVel = 0
		if (i.pitchup) {
			state.pitchVel = MAN_PITCHVEL * 0.5
		} else if (i.pitchdown) {
			state.pitchVel = -MAN_PITCHVEL * 0.5
		}
		state.pitch += state.pitchVel * ft
		...
		if (i.forward) {
			state.vel.x = MAN_VEL * Math.cos(state.yaw) * MOVE_SPEED
			state.vel.y = MAN_VEL * Math.sin(state.yaw) * MOVE_SPEED
		} else if (i.back) {
			state.vel.x = -MAN_VEL * Math.cos(state.yaw) * MOVE_SPEED
			state.vel.y = -MAN_VEL * Math.sin(state.yaw) * MOVE_SPEED
		}
		...
		//Vec3.set(state.vel, 0, 0, 0)
		if (i.straftLeft) {
			state.vel.y = MAN_VEL * Math.sin(state.yaw + (Math.PI / 2)) * MOVE_SPEED
			state.vel.x = MAN_VEL * Math.cos(state.yaw + (Math.PI / 2)) * MOVE_SPEED
		} else if (i.straftRight) {
			state.vel.y = -MAN_VEL * Math.sin(state.yaw + (Math.PI / 2)) * MOVE_SPEED
			state.vel.x = -MAN_VEL * Math.cos(state.yaw + (Math.PI / 2)) * MOVE_SPEED
		}
		state.pos.x += state.vel.x * ft
		...
		else if (state.pos.z > groundHeight) {
			state.pos.z = groundHeight + MIN_HEIGHT + VIEW_HEIGHT;
		} 
		else if (state.pos.z > MAX_HEIGHT) {
			state.pos.z = MAX_HEIGHT
		}
